...
for(i1 = 0; i1 < 8; i1 += 1) {
 ...
 for(i2 = 0; i2 < 8; i2 += 4) {
  for(i3 = 0; i3 < 8; i3 += 1) {
   ...
   Mu[i1][i2 + 1][i3] = M123[i1][i2 + 1][i3] * u[e][i1][i2 + 1][i3];
   Mu[i1][i2 + 2][i3] = M123[i1][i2 + 2][i3] * u[e][i1][i2 + 2][i3];
   Mu[i1][i2 + 3][i3] = M123[i1][i2 + 3][i3] * u[e][i1][i2 + 3][i3];
   }
  }
 }
for(j1 = 0; j1 < 8; j1 += 1) {
 ...
 for(j2 = 0; j2 < 8; j2 += 4) {
  for(j3 = 0; j3 < 8; j3 += 1) {
   for(j4 = 0; j4 < 8; j4 += 1) {
    ...
    cr1[j1][j2 + 1][j3] += Lh[j1][j4] * Mu[j4][j2 + 1][j3];
    cr1[j1][j2 + 2][j3] += Lh[j1][j4] * Mu[j4][j2 + 2][j3];
    cr1[j1][j2 + 3][j3] += Lh[j1][j4] * Mu[j4][j2 + 3][j3];
    }
   }
  }
 }
 ...
 for(k2 = 0; k2 < 8; k2 += 4) {
  for(k3 = 0; k3 < 8; k3 += 1) {
   for(k4 = 0; k4 < 8; k4 += 1) {
    ...
    cr2[k1][k2 + 1][k3] += Lh[k1][k4] * Mu[k2 + 1][k4][k3];
    cr2[k1][k2 + 2][k3] += Lh[k1][k4] * Mu[k2 + 2][k4][k3];
    cr2[k1][k2 + 3][k3] += Lh[k1][k4] * Mu[k2 + 3][k4][k3];
    }
   }
  }
 }
 ...
 for(l2 = 0; l2 < 8; l2 += 4) {
  for(l3 = 0; l3 < 8; l3 += 1) {
   for(l4 = 0; l4 < 8; l4 += 1) {
    ...
    cr3[l1][l2 + 1][l3] += Lh[l1][l4] * Mu[l2 + 1][l3][l4];
    cr3[l1][l2 + 2][l3] += Lh[l1][l4] * Mu[l2 + 2][l3][l4];
    cr3[l1][l2 + 3][l3] += Lh[l1][l4] * Mu[l2 + 3][l3][l4];
    }
   }
  }
 }
